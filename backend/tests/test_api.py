...
class TestPlanRailAPI(unittest.TestCase):
    ...
    def test_unimplemented_contracts(self):
        res_opt = self.client.post("/api/v1/optimization/generate", json={"target_date": "2026-09-10"})
        self.assertEqual(res_opt.status_code, 501)

        res_sim = self.client.post("/api/v1/simulation/run", json={"selected_request_ids": ["MR0001"]})
        self.assertEqual(res_sim.status_code, 501)
