...
class WindowService:
    ...
    def get_maintenance_windows(
        ...
        total = query.count()
        windows = query.order_by(MaintenanceWindow.window_id.asc()).offset((page - 1) * page_size).limit(page_size).all()
    ...
    def get_traffic_windows(
        ...
        total = query.count()
        tw = query.order_by(TrafficWindow.traffic_window_id.asc()).offset((page - 1) * page_size).limit(page_size).all()
        return tw, total
